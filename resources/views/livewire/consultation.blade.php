...
<div>
    <x-card title="">
        <div>
            ...
            <div class="input-group">
                ...
                <input wire:model="age" class="form-control" type="number" readonly>
            ...
            @if (!$is_open)
                <div class="d-flex justify-content-between mt-2 mb-1">
                    ...
                    <button wire:click="openDiagnostic" class="btn btn-success"
                        {{ Auth::user()->role != Role::MEDIC ? 'disabled' : '' }}
                        {{ $filled ? 'disabled' : '' }}>Nuevo Diagnostico</button>
                </div>
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($this->getDiagnostics() as $diag)
                            <tr
                                @if (Auth::user()->role == Role::MEDIC) wire:click="getDiagnostic({{ $diag->id }})" class="select-item" @endif>
                                <td>{{ $diag->id }}</td>
                                <td>{{ Carbon::parse($diag->updated_at)->toDateString() }}</td>
                                <td>{{ $diag->description }}</td>
                                <td>
                                    <i
                                        class="fa fa-power-off @if ($diag->status == 1) {{ 'text-success' }} @else {{ 'text-danger' }} @endif"></i>
                                </td>
                                <td>
            ...
            @else
                ...
                <div class="d-flex">
                    ...
                    <div style="width: 75%;margin-left: 5px;">
                        <div class="mb-3">
                            ...
                            <textarea wire:model="consultation" class="form-control" rows="2" cols=""
                                {{ $filled ? 'readonly' : '' }}></textarea>
                            @error('consultation')
                                <span class="input-group-text text-bg-danger" data-bs-toggle="tooltip"
                                    data-bs-title="{{ $message }}">
                                    <i class="fa fa-exclamation"></i>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea wire:model="physicalExam" class="form-control" rows="2" cols=""
                                {{ $filled ? 'readonly' : '' }}></textarea>
                            @error('physicalExam')
                                <span class="input-group-text text-bg-danger" data-bs-toggle="tooltip"
                                    data-bs-title="{{ $message }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea wire:model="detail_diagnostic" class="form-control" rows="3" cols=""
                                {{ $filled ? 'readonly' : '' }}></textarea>
                ...
                <div class="mb-1 d-flex justify-content-between">
                    ...
                    <a data-bs-toggle="modal" data-bs-target="#modal"
                        class="btn btn-success {{ $filled ? 'd-none' : '' }}">
                    ...
                    <tbody>
                        ...
                        @foreach ($treatments as $key => $treatment)
                            <tr>
                                ...
                                <td>{{ $treatment['by_day'] }}</td>
                                <td>
                                    <input wire:model.live="treatment_new" class="form-check-input" type="radio"
                                        name="treatment_active" value="{{ $key }}">
                                </td>
                                <td>
                                    <button wire:click="deleteTreatment({{ $key }})" class="btn btn-danger">
                ...
                <div class="mb-3">
                    ...
                    <textarea wire:model="evaluation" class="form-control" rows="" cols=""
                        {{ $filled ? 'readonly' : '' }}></textarea>
                    @error('evaluation')
                        <span class="input-group-text text-bg-danger" data-bs-toggle="tooltip"
                            data-bs-title="{{ $message }}">
                ...
                <h5 class="card-text">Reservaciones</h5>
                <div class="input-group">
                    <span class="input-group-text">Dia</span>
                    <input wire:model.live="date" class="form-control" type="date">
                    <span class="input-group-text">Hora</span>
                    <select wire:model.live="schedule" class="form-select">
                        <option value="null">Seleccione</option>
                        @foreach ($this->getSchedules() ?? [] as $item)
                            <option value="{{ $item->id }}">
                                {{ $item->schedule->start . '-' . $item->schedule->end }}</option>
                        @endforeach
                    </select>
                    @error('schedule')
                        <span class="input-group-text text-bg-danger" data-bs-toggle="tooltip"
                    ...
                    @enderror
                    <button wire:click="addReservation" class="btn btn-success">
                        <i class="fa fa-plus"></i>
                    </button>
                </div>
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($reservations ?? [] as $item)
                            @if (
                                $item->staffSchedule->schedule->day->value == Carbon::now()->dayOfWeek and
                                    Carbon::parse($item->staffSchedule->schedule->start) >= Carbon::now()->todateTime())
                                <tr>
                                    <td>{{ $item->date }}</td>
                                    <td>{{ $item->staffSchedule->schedule->start }}</td>
                                    <td>
                                        <a wire:click="deleteSaveReservation({{ $item->id }})"
                            ...
                            @endif
                            @if ($item->staffSchedule->schedule->day->value > Carbon::now()->dayOfWeek)
                                <tr>
                                    <td>{{ $item->date }}</td>
                                    <td>{{ $item->staffSchedule->schedule->start }}</td>
                                    <td>
                        ...
                        @foreach ($schedules ?? [] as $key => $item)
                            <tr>
                                <td>{{ $item['date'] }}</td>
                                <td>{{ Staff_schedule::find($item['staff_schedule_id'])->schedule->start }}</td>
                                <td>
                                    <a wire:click="deleteReservation({{ $key }})" class="btn btn-danger">
                                        <i class="fa fa-trash"></i>
                                    </a>
                                </td>
                            </tr>
                ...
                <div class="input-group">
                    ...
                    <input wire:model.live="canceled" type="number" class="form-control"
                        placeholder="{{ $canceled_placeholder }}">
                </div>
            @endif
        </div>
        <x-slot name="footer">
            <div class="d-flex justify-content-end">
                @if ($diagnostic != null)
                    <a wire:click="toogleFinish"
                        class="btn btn-{{ $diagnostic->status ? 'primary' : 'secondary' }} mx-1">{{ $diagnostic->status ? 'Finalizar' : 'Finalizado' }}</a>
                @endif
    ...
    <a onclick="window.open('{{ route('dashboard.historyPatient', $person_id) }}', '_blank','width=800,height=600')"
        class="float">
        <i class="my-float nf nf-fa-list"></i>
    </a>
    <x-modal id="modal" title="Añadir Tratamiento">
        <livewire:add-treatment :ci="$ci"></livewire:add-treatment>
    </x-modal>
</div>
